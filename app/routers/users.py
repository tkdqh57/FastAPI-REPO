...
async def following_user(
    ...
    follow, _ = await Follow.get_or_create(follow_id=user.id, following_id=user_id)
...
async def unfollowing_user(
    ...
    follow = await Follow.filter(follower_id=user.id, following_id=user_id)
...
async def get_my_followings(user: Annotated[User, Depends(get_current_user)])
...
async def get_my_followers(user: Annotated[User, Depends(get_currend_user)])
    following_users = await User.filter(
